    ...
    for (i <- 0 to LatSegs-1; j <- 0 to LongSegs-1) {    // iterate through the tiles
      ...
      println("Number of records found in the cell: " + numRecs)
        ...
        if (numRecs > 0) {
          ...
          tileStats += calcShipsPerTimeOfDay(grid(i)(j), grid(i + 1)(j + 1), ais10).withColumn("A_lat", lit(grid(i)(j)._1)).withColumn("A_lon", lit(grid(i)(j)._2)).withColumn("B_lat", lit(grid(i + 1)(j + 1)._1)).withColumn("B_lon", lit(grid(i + 1)(j + 1)._2))
          if (DFcounter > 100) {
            println ("Temp. saving of data Reducing the number of Dataframes from:"+ tileStats.size)
            val ShipsPerTimeOfDay = tileStats.reduce((a,b)=> a.union(b))
            ShipsPerTimeOfDay
              .coalesce(1)
