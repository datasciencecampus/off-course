...
object GridStatsSPD {
  def main(args: Array[String]): Unit = {
    ...
    for (i <- 0 to LatSegs-1; j <- 0 to LongSegs-1) {    // iterate through the tiles
      ...
      val numRecs:Long = ais10.count

      println("Number of records found in the cell: " + numRecs)

       if (numRecs > 0 ) {
        println("calcShipsPerDay ...#:"+DFcounter)
        ...
        tileStats += calcShipsPerDay(grid(i)(j), grid(i + 1)(j + 1), ais10).withColumn("A_lat", lit(grid(i)(j)._1)).withColumn("A_lon", lit(grid(i)(j)._2)).withColumn("B_lat", lit(grid(i + 1)(j + 1)._1)).withColumn("B_lon", lit(grid(i + 1)(j + 1)._2))
        DFcounter += 1
        ...
        if (DFcounter > 100) {
          ...
          val ShipsPerDay = tileStats.reduce((a,b)=> a.union(b))
    ...
    println ("Final Reducing the number of Dataframes from:"+ DFcounter)

    //val broadcastedList = spark.sparkContext.broadcast(tileStats)
